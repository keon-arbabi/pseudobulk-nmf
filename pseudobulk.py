...
def cell_type_annotation(sc, study_name, original_labels_column, directory):
    ...
    with Timer(f'[{study_name}] PCA'):
        sc, sc_ref = sc.PCA(sc_ref, verbose=True)
    ...
    with Timer(f'[{study_name}] Plots'):
        sc.plot_embedding('cell_type_fine', 
            f'{directory}/cell_type_fine_umap.png',
...
if os.path.exists(sc_file):
    ...
    with Timer(f'[{study_name}] Preprocessing single cell'):
        ...
        rosmap_meta = pl.read_csv(
            f'{sc_dir}/dataset_978_basic_04-21-2023_with_pmAD.csv',
            schema_overrides={'projid': pl.String})\
            .unique(subset='projid') 
        full_meta = basic_meta\
            ...
            .join(rosmap_meta, on='projid', how='left', coalesce=True)
